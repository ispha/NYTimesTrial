...
extension ArticlesViewController : UITableViewDelegate, SkeletonTableViewDataSource
{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return HelpingMethods.emptyTableView(tableView: tableView, dataCount: arrayOfItems.count, dataCome: true, emptyTableViewMessage: "No articles Found!")
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if arrayOfItems.count > 0
        {
            ...
            cell.hideSkeleton()
            let article = arrayOfItems[indexPath.row]
            cell.configureCellWithPlaceItem(item: article)
        }
        else
        {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        {
            let detailsVC = UIStoryboard(name: Constants.StoryboardIDs.Main, bundle: nil).instantiateViewController(identifier: Constants.StoryboardVCsIDs.ArticleDetailsViewController) as! ArticleDetailsViewController
            detailsVC.articleModel = arrayOfItems[indexPath.row]
